...
class M_pelanggan extends CI_Model
{
	...
	{
		return $this->db
			->select('id_pelanggan, nama, alamat, telp, status_anggota, info_tambahan')
			->order_by('nama','asc')
			->get('pj_pelanggan');
	}
	...
	{
		return $this->db
			->select('id_pelanggan, nama, alamat, telp, info_tambahan, status_anggota')
	...
	function fetch_data_pelanggan($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		...
		if( ! empty($like_value))
		{
			...
			$sql .= "
				...
				OR a.`status_anggota` LIKE '%".$this->db->escape_like_str($like_value)."%' 
	...
	function update_pelanggan($id_pelanggan, $nama, $alamat, $telepon, $info, $status)
	{
		$dt = array(
			'nama' => $nama,
			'alamat' => $alamat,
	...
	{
		return $this->db->query("DELETE from pj_pelanggan WHERE id_pelanggan =".$id_pelanggan.";");
	}
