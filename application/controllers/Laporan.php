...
class Laporan extends MY_Controller 
{
	...
	{
		$this->load->view('laporan/form_keuntungan');
	}	

	public function keuntungan($from, $to)
	{
		$this->load->model('m_penjualan_master');
		$dt['penjualan'] 	= $this->m_penjualan_master->laporan_keuntungan($from, $to);
		$dt['from']			= date('d F Y', strtotime($from));
		$dt['to']			= date('d F Y', strtotime($to));
		$this->load->view('laporan/laporan_keuntungan', $dt);
	}
	...
	public function exceluntung($from, $to)
	{
		$this->load->model('m_penjualan_master');
		$penjualan 	= $this->m_penjualan_master->laporan_keuntungan($from, $to);
		if($penjualan->num_rows() > 0)
		{
			$filename = 'Laporan_Keuntungan_'.$from.'_'.$to;
			header("Content-type: application/x-msdownload");
			header("Content-Disposition: attachment; filename=".$filename.".xls");

			echo "
				<h4>Laporan Keuntungan Tanggal ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to))."</h4>
				<table border='1' width='100%'>
					<thead>
						<tr>
							<th>No</th>
	...
	public function pdfuntung($from, $to)
	{
		$this->load->library('cfpdf');
		...
		$pdf->SetFont('Arial','',10);
		$pdf->Cell(0, 8, "Laporan Keuntungan Tanggal ".date('d/m/Y', strtotime($from))." - ".date('d/m/Y', strtotime($to)), 0, 1, 'L'); 

		$pdf->Cell(15, 7, 'No', 1, 0, 'L'); 
		$pdf->Cell(85, 7, 'Tanggal', 1, 0, 'L');
		$pdf->Cell(85, 7, 'Total Keuntungan', 1, 0, 'L'); 
		$pdf->Ln();
		...
		foreach($penjualan->result() as $p)
		{
			$pdf->Cell(15, 7, $no, 1, 0, 'L'); 
			$pdf->Cell(85, 7, date('d F Y', strtotime($p->tanggal)), 1, 0, 'L');
			$pdf->Cell(85, 7, "Rp. ".str_replace(",", ".", number_format($p->total_keuntungan)), 1, 0, 'L');
			$pdf->Ln();
		...
		$pdf->Cell(100, 7, 'Total Seluruh Keuntungan', 1, 0, 'L'); 
		$pdf->Cell(85, 7, "Rp. ".str_replace(",", ".", number_format($total_keuntungan)), 1, 0, 'L');
		$pdf->Ln();
